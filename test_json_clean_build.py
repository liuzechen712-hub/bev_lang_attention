...
model_cfg = {
    ...
    'pts_bbox_head': {
        ...
        'num_classes': 10, 'in_channels': 256, 'num_query': 900,
...
# 2. 关键步骤：JSON 序列化 -> 反序列化
#    这会彻底丢弃所有 ConfigDict 的元数据，只保留纯数据
print("🧼 Sanitizing config via JSON round-trip...")
json_str = json.dumps(model_cfg)
clean_dict = json.loads(json_str)

# 3. 重新包装为 ConfigDict
final_cfg = ConfigDict(clean_dict)

print("🔨 Building model with JSON-cleaned ConfigDict...")
try:
